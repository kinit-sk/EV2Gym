...
def load_ev_spawn_scenarios(env) -> None:
    ...
    env.time_of_connection_vs_hour = np.load(
        time_of_connection_vs_hour_file
    )  # time of connection vs hour

    env.df_req_energy = pd.read_csv(df_req_energy_file)  # energy demand per arrival
    # replace column work with workplace
    env.df_req_energy = env.df_req_energy.rename(columns={"work": "workplace", "home": "private"})
    env.df_req_energy = env.df_req_energy.fillna(0)

    env.df_time_of_stay_vs_arrival = pd.read_csv(
        df_time_of_stay_vs_arrival_file
    )  # time of stay vs arrival
    env.df_time_of_stay_vs_arrival = env.df_time_of_stay_vs_arrival.fillna(0)
    env.df_time_of_stay_vs_arrival = env.df_time_of_stay_vs_arrival.rename(
        columns={"work": "workplace", "home": "private"}
    )
...
def generate_residential_inflexible_loads(env) -> np.ndarray:
    ...
    if desired_timescale > dataset_timescale:
        ...
        data = data.loc[data.index.repeat(dataset_timescale / desired_timescale)].reset_index(
            drop=True
        )
    ...
    for i in range(number_of_transformers):
        new_data["tr_" + str(i)] = data.sample(10, axis=1, random_state=env.tr_seed).sum(axis=1)
...
def generate_pv_generation(env) -> np.ndarray:
    ...
    data["electricity"] = (
        data["electricity"].rolling(window=60 // desired_timescale, min_periods=1).mean()
    )
        ...
        for i, tr in enumerate(env.charging_network_topology):
            for cs in env.charging_network_topology[tr]["charging_stations"]:
                ev_charger = EV_Charger(
                    ...
                    min_charge_current=env.charging_network_topology[tr]["charging_stations"][cs][
                        "min_charge_current"
                    ],
                    max_charge_current=env.charging_network_topology[tr]["charging_stations"][cs][
                        "max_charge_current"
                    ],
                    min_discharge_current=env.charging_network_topology[tr]["charging_stations"][
                        cs
                    ]["min_discharge_current"],
                    max_discharge_current=env.charging_network_topology[tr]["charging_stations"][
                        cs
                    ...
                    charger_type=env.charging_network_topology[tr]["charging_stations"][cs][
                        "charger_type"
                    ],
...
def load_grid(env):
    ...
    if env.simulate_grid:
        ...
        env.cs_transformers = [*np.arange(env.number_of_transformers)] * (
            env.cs // env.number_of_transformers
        )
        env.cs_transformers += np.arange(env.cs % env.number_of_transformers).tolist()
        # print(f'cs_transformers: {env.cs_transformers}')

        assert env.charging_network_topology is None, (
            "Charging network topology is not supported with grid simulation."
        )
